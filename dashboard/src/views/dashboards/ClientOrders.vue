<template>
  <div class="p-8 w-full">
    ...
    <div v-else>
      ...
      <!-- Pagination -->
      <div v-if="filteredOrders.length > 0" class="flex items-center justify-between mt-4 text-sm text-background-500">
        <span>{{ t('common.showing', { from: paginationFrom, to: paginationTo, total: filteredOrders.length }) }}</span>
...
const filteredOrders = computed(() => {
  ...
  return orders.value.filter(o => {
    const matchesQuery = !q || o.orderNumber.toLowerCase().includes(q) || o.clientName.toLowerCase().includes(q)
...
const paginationFrom = computed(() => filteredOrders.value.length === 0 ? 0 : (currentPage.value - 1) * pageSize.value + 1)
const paginationTo = computed(() => Math.min(currentPage.value * pageSize.value, filteredOrders.value.length))
