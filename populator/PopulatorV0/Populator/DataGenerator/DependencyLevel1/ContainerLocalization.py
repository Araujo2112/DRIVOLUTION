...
def generate_container_localization_histories(num: int) -> List[dict]:
    """
    Para cada contentor, escolhe aleatoriamente se associa às secções em ordem sequencial ou embaralhada (shuffle).
    """
    ...
    if not container_ids or not section_ids:
        raise ValueError("É necessário ter containers e seções no contexto!")

    histories = []

    for container_id in container_ids:
        #modo = random.choice(['sequencial', 'shuffle'])
        modo = random.choice(['sequencial'])
    ...
    try:
        ...
        if response.status_code in (200, 201):
            ...
            context.add_entity(
                ...
                entity_id=created_history['id'],  # pode ser igual ao db_id
                entity_data=created_history,
                db_id=created_history['id']  # id da base de dados
            )
        return response
